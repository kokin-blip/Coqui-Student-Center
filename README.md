...
- In-app academic calendar refresh over a DNS-pinned, redirect-blocking, private-network-blocking client. It returns a reviewable diff and never writes: a changed term date is raised as an explicit conflict. Setup works identically with no network, on the bundled snapshot.
