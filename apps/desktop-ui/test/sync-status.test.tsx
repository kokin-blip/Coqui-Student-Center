...
const openAccount = async () => {
  ...
  await user.click(
    await screen.findByRole("button", { name: "Settings" }, timeout),
  );
  await user.click(
    await screen.findByRole("button", { name: /Account & sync/ }, timeout),
  );
  return user;
};
...
test("staged changes from a newer version are described honestly", async () => {
  ...
  expect(
    screen.queryByText(/Downloaded changes are safely staged/i),
  ).toBeNull();
  expect(
...
test("the staged-changes notice is absent when nothing is waiting", async () => {
  ...
  await screen.findByRole(
    "heading",
    { name: "Optional Student Center account" },
    timeout,
  );
