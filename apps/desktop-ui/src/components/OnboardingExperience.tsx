      ...
      <section className="setup-panel">
        ...
        <div className="setup-stage" key={step}>
          ...
          {step === 1 && <>
            ...
            {schools.length > 0 && <div className="prediction-list" role="listbox" aria-label="School suggestions">{schools.map((school) => <button key={school.id} className={draft.institution.id === school.id ? "selected" : ""} onClick={() => { update("institution", { ...school, campusId: "", campusName: "" }); setSchoolQuery(school.name); setSchools([]); }}><School /><span><strong>{school.name}</strong><small>{school.custom ? "Custom worldwide school" : `${school.country} institution directory`}</small></span>{draft.institution.id === school.id && <Check />}</button>)}</div>}
            ...
            {Boolean(institutionOptions?.campuses.length) && <fieldset className="setup-fieldset institution-options"><legend>Primary campus</legend><p className="field-help">Choose where you attend most often. Individual classes can still use another campus.</p><div className="campus-options">{institutionOptions!.campuses.map((campus) => <button key={campus.id} className={draft.institution.campusId === campus.id ? "selected" : ""} onClick={() => update("institution", { ...draft.institution, campusId: campus.id, campusName: campus.name })}><MapPin /><span><strong>{campus.name}</strong><small>{campus.city}</small></span>{draft.institution.campusId === campus.id && <Check />}</button>)}</div><small className="source-note">Campus source: {institutionOptions!.campuses[0].sourceLabel}</small></fieldset>}
